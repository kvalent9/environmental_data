...
cewa_present_absent <- as.numeric(dat_all$AMCR > 1)
cewa_present_absent
...
recr_present_absent <- as.numeric(dat_all$AMRO > 1)
recr_present_absent

plot(x = dat_all$elev, y= recr_present_absent, xlab= "Elevation") 
curve(logistic_midpoint_slope(x, midpoint = 400, slope = -0.2), add = TRUE)

boolean_jay <- dat_all$GRJA > 0
dat_all$GRJA[boolean_jay]
...
plot(main= "Impact of Elevation on Crow Occurrence", x = dat_all$elev, y= recr_present_absent, xlab= "Elevation", ylab = "Crow Occurrence") 
curve(logistic_midpoint_slope(x, midpoint = 400, slope = -0.2), add = TRUE)
